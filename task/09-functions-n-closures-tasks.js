...
function getPolynom(...arr) {
    return (x)=>{
        ...
        arr.forEach((element, index) => {
            result += element*(x**(arr.length-(++index)))
        });
        return result;
    }
}
...
function logger(func, logFunc) {
    return (...args)=>{
        let stringArr = JSON.stringify(args);
        stringArr = stringArr.slice(1,-1);
        logFunc(`${func.name}(${stringArr}) starts`);
        let result = func(...args);
        logFunc(`${func.name}(${stringArr}) ends`);
        return result;
    }
}
...
function partialUsingArguments(fn,...args) {
    return function(...remArgs){
        return fn.bind(this,...args)(...remArgs);
    }
}
