...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
    let resultString = "";
    resultString += (isStartIncluded)?"[":"(";
    resultString += `${Math.min(a,b)}, ${Math.max(a,b)}`;
...
function isBracketsBalanced(str) {
    ...
    for(let char of str){
        ...
        if(char === '[' || char === '(' || char === '{' || char === '<')
            stack.push(char);
        else if((char === ']' && top === '[') || (char === '}' && top === '{') || (char === ')' && top === '(') ||(char === '>' && top === '<'))
            stack.pop();
        else
            return false;
    }
...
function getMatrixProduct(m1, m2) {
    ...
    for(let row=0;row<m1.length;row++){
        resultMatrix[row] = new Array(m2[0].length).fill(0);
